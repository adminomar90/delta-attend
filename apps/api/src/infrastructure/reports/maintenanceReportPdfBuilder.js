/**
 * Maintenance Report PDF — rebuilt on the unified template.
 */
...
const STATUS_MAP = {
  ...
  SUBMITTED:            { ar: 'بانتظار المراجعة', bg: COLORS.warning },
  MANAGER_APPROVED:     { ar: 'معتمد من المدير',  bg: COLORS.success },
  MANAGER_REJECTED:     { ar: 'مرفوض من المدير',  bg: COLORS.danger  },
  COMPLETED:            { ar: 'مكتمل',           bg: COLORS.success },
  FEEDBACK_SUBMITTED:   { ar: 'تم تقييم الزبون', bg: COLORS.accent  },
};
...
export const buildMaintenanceReportPdfBuffer = async (
  ...
  drawTableRow(ctx, 'الفني',           safe(report?.assignedEmployee?.fullName || report?.assignedEmployeeName), idx++);
  ...
  drawBulletList(ctx, report?.maintenanceTypes || [], 'لم يتم تحديد نوع الصيانة');
  ...
  if (devicesCount) {
    drawDataTable(ctx, {
      ...
      rows: (report.inspectedDevices || []).map((d, i) => [
        `${i + 1}`, safe(d.device), safe(d.model), safe(d.condition), safe(d.notes),
      ]),
    });
  } else {
  ...
  if (issuesCount) {
    drawDataTable(ctx, {
      ...
      rows: (report.detectedIssues || []).map((d, i) => [
        safe(d.sequenceNo || i + 1), safe(d.issue), safe(d.severity), safe(d.proposedSolution),
      ]),
    });
  } else {
  ...
  if (report?.customerFeedback?.submittedAt) {
    ...
    drawTableRow(ctx, 'نوع المشروع',   safe(report?.customerFeedback?.projectType), idx++);
    drawTableRow(ctx, 'تقييم الشركة',   safe(report?.customerFeedback?.companyRating), idx++);
    drawTableRow(ctx, 'تقييم الموظف',   safe(report?.customerFeedback?.employeeRating), idx++);
  ...
  ctx.doc.moveDown(0.4);
  drawSectionTitle(ctx, 'سجل الإجراءات');
  ...
  if (!trail.length) {
    ...
    trail.forEach((entry, i) => {
      drawTimelineEntry(ctx, {
        ...
        beforeStatus: safe(entry.beforeStatus),
        afterStatus: safe(entry.afterStatus),
        notes: entry.notes || '',
      });
    });
  }
