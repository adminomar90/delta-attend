...
import { buildDailyWorkPlanWhatsappMessage } from '../../../lib/dailyWorkPlanWhatsapp';
...
const enrichPlan = (plan, { currentUserId = '', canManage = false } = {}) => {
  ...
  const displayProgressPercent = actorAssignee
    ? Number(actorAssignee.progressPercent || 0)
    : Number(plan.progressPercent || 0);
...
export default function DailyWorkPlansPage() {
  ...
  const load = async (silent = false) => {
    ...
    try {
      ...
      setPlans((response.plans || []).map((plan) => enrichPlan(plan, {
  ...
  const loadCalendar = async (silent = false) => {
    ...
    try {
      ...
      setCalendarPlans((response.plans || []).map((plan) => enrichPlan(plan, {
        currentUserId: currentUser?.id,
        canManage,
      })));
    } catch (err) {
      if (!silent) {
        setError(err.message || 'تعذر تحميل تقويم البلان اليومي');
      }
    } finally {
      if (!silent) {
  ...
  useEffect(() => {
    if (!canView || !calendarOpen) return;
    loadCalendar();
  }, [calendarOpen, calendarQueryString, canView]);
  ...
  useEffect(() => {
    if (!filters.planDate) return;
    setCalendarSelectedDate(filters.planDate);
    setCalendarMonth(createMonthAnchor(filters.planDate));
  }, [filters.planDate]);

  useEffect(() => {
    if (!canView || !calendarOpen) return undefined;
  ...
  useEffect(() => {
    ...
    const rafId = window.requestAnimationFrame(() => {
      const calendarElement = calendarSectionRef.current;
      if (!calendarElement) return;

      const topOffset = 88;
      const targetTop = calendarElement.getBoundingClientRect().top + window.scrollY - topOffset;
      window.scrollTo({
  ...
  const handleSavePlan = async (form) => mutate(async () => {
    ...
    if (editingPlan?._id || editingPlan?.id) {
      await api.patch(`/daily-work-plans/${editingPlan._id || editingPlan.id}`, formData);
    } else {
      await api.post('/daily-work-plans', formData);
    }
  ...
  const handleActionSubmit = async (payload) => {
    ...
    if (actionState.mode === 'progress') {
      return mutate(() => api.patch(`/daily-work-plans/${planId}/progress`, payload), 'تم تحديث نسبة الإنجاز.');
    }
    if (actionState.mode === 'status') {
      return mutate(() => api.patch(`/daily-work-plans/${planId}/status`, payload), 'تم تحديث الحالة.');
    }
    if (actionState.mode === 'postpone') {
      return mutate(() => api.post(`/daily-work-plans/${planId}/postpone`, payload), 'تم تأجيل البلان.');
    }
    if (actionState.mode === 'approve') {
      return mutate(() => api.patch(`/daily-work-plans/${planId}/approve`, payload), 'تم اعتماد البلان.');
    }
    if (actionState.mode === 'finish') {
      return mutate(() => api.patch(`/daily-work-plans/${planId}/status`, payload), 'تم إرسال البلان للاعتماد.');
    }
    return null;
  };

  const quickStatusUpdate = async (plan, status) => mutate(
    () => api.patch(`/daily-work-plans/${plan._id || plan.id}/status`, buildStatusPayload({ status })),
    'تم تحديث الحالة بنجاح.',
  );

  const deletePlan = async (plan) => {
    if (!window.confirm(`هل تريد حذف البلان "${plan.title}"؟`)) return;
    await mutate(() => api.delete(`/daily-work-plans/${plan._id || plan.id}`), 'تم حذف البلان.');
  };

  const rolloverPlan = async (plan) => {
    await mutate(() => api.post(`/daily-work-plans/${plan._id || plan.id}/rollover`, {}), 'تم ترحيل البلان إلى اليوم التالي.');
  };
  ...
  const downloadFile = async (format) => {
    ...
    try {
      ...
      link.href = url;
      link.download = `daily-work-plans.${format === 'excel' ? 'xlsx' : 'pdf'}`;
      link.click();
  ...
  const handleCalendarMonthChange = (nextMonth) => {
    ...
    const sameMonth = (
      selectedAnchor.getUTCFullYear() === nextAnchor.getUTCFullYear()
  ...
  const isAssignedToMe = (plan) => (plan.assignees || []).some((assignee) => String(assignee.user?._id || assignee.user?.id || assignee.user) === currentUser?.id);

  const isTeamLeaderOfPlan = (plan) => {
    const leaderId = plan.teamLeader?._id || plan.teamLeader?.id || plan.teamLeader || '';
    return String(leaderId) === String(currentUser?.id || '');
  };

  if (!canView) {
    return <section className="card section">لا تملك صلاحية الوصول إلى بلان العمل اليومي.</section>;
  }
    ...
    <>
      ...
      <section className="card section daily-plan-hero">
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--text-soft)' }}>تنظيم المهام اليومية، متابعة التنفيذ، والتحديث المباشر حسب الصلاحيات الحالية.</p>
        </div>
        <div className="action-row">
          {canCreate ? <button className="btn btn-primary" onClick={() => setEditingPlan({})}>إضافة بلان جديد</button> : null}
          <button className="btn btn-soft" onClick={handleCalendarToggle}>
      ...
      <section className="card section" style={{ marginTop: 16 }}>
        <div className="daily-plan-toolbar">
          <div className="daily-plan-filter-grid">
            <label>بحث عام<input className="input" value={filters.search} onChange={(e) => setFilters((prev) => ({ ...prev, search: e.target.value }))} /></label>
          ...
          <div className="action-row">
            ...
            <button className="btn btn-soft" onClick={() => setViewMode((prev) => (prev === 'cards' ? 'table' : 'cards'))}>
              {viewMode === 'cards' ? 'عرض جدولي' : 'عرض بطاقات'}
            </button>
          </div>
        </div>
      </section>
      ...
      {productivity.length ? (
        <section className="card section" style={{ marginTop: 16 }}>
          <div className="section-header">
            ...
            <span style={{ color: 'var(--text-soft)', fontSize: 13 }}>يظهر أعلى 5 موظفين ضمن نطاقك الحالي</span>
          </div>
        ...
        {viewMode === 'cards' ? (
          <div className="daily-plan-card-grid">
            {plans.length ? plans.map((plan) => (
              <article key={plan._id || plan.id} className="daily-plan-card">
                ...
                <div className="daily-plan-mini-grid">
                  ...
                  <div><span>النوع</span><strong>{plan.taskTypeLabel}</strong></div>
                  <div><span>آخر تحديث</span><strong>{formatDateTime(plan.lastUpdatedAt)}</strong></div>
                </div>
                <div className="daily-plan-progress-row">
                  <span>{plan.displayProgressLabel || 'التقدم'}</span>
                  <ProgressGauge value={plan.displayProgressPercent || 0} />
                </div>
                <div className="daily-plan-card-subtitle">
                  قائد الفريق: {plan.teamLeader?.fullName || 'غير محدد'}
                </div>
                <div className="daily-plan-card-subtitle">
                  المكلفون: {formatAssigneesSummary(plan.assignees, { maxVisible: 2 })}
                </div>
                {plan.isApproved ? (
                  <div className="daily-plan-card-subtitle">
                    إجمالي النقاط الممنوحة: {plan.pointsAwardedTotal || 0}
                  </div>
                ) : null}
                {plan.postponedTo ? (
                  <div className="daily-plan-card-subtitle">
                    مؤجلة إلى: {formatDateTime(plan.postponedTo)}
                  </div>
                ) : null}
            ...
            )) : <p className="maintenance-empty">لا توجد بلانات مطابقة للفلاتر الحالية.</p>}
          </div>
        ) : (
          <div className="work-reports-table-shell">
            <table className="table">
              <thead><tr><th>العنوان</th><th>الموظفون</th><th>التاريخ</th><th>الحالة</th><th>الأولوية</th><th>الإنجاز</th><th>إجراءات</th></tr></thead>
              <tbody>
                {plans.map((plan) => (
                  <tr key={plan._id || plan.id}>
                    <td><strong>{plan.title}</strong><div style={{ color: 'var(--text-soft)', fontSize: 12 }}>{plan.customerName || plan.project?.name || '-'} - {plan.location || '-'}</div></td>
                    ...
                    <td><span className={`status-pill ${dailyWorkPlanStatusClassMap[plan.status] || 'status-todo'}`}>{plan.statusLabel}</span></td>
                    <td><span className={`status-pill ${dailyWorkPlanPriorityClassMap[plan.priority] || 'status-todo'}`}>{plan.priorityLabel}</span></td>
                    <td>
                      <ProgressGauge value={plan.displayProgressPercent || 0} />
                      {!canManage && plan.actorAssignee ? (
                        <div style={{ color: 'var(--text-soft)', fontSize: 12, marginTop: 6 }}>
                          الإنجاز العام: {plan.progressPercent || 0}%
                        </div>
                      ) : null}
                    </td>
                    <td><div className="form-actions"><button className="btn btn-soft btn-sm" onClick={() => setDetailsPlan(plan)}>تفاصيل</button>{canManage ? <button className="btn btn-soft btn-sm" onClick={() => setEditingPlan(plan)}>تعديل</button> : null}</div></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <DailyWorkPlanModal
        open={!!editingPlan}
        initialForm={editingPlan}
        users={users}
        projects={projects}
        saving={saving}
        title={editingPlan?._id || editingPlan?.id ? 'تعديل بلان العمل اليومي' : 'إضافة بلان عمل يومي'}
        subtitle={editingPlan?.title || 'إنشاء خطة يومية جديدة وتوزيعها على الموظفين'}
        onClose={() => setEditingPlan(null)}
        onSubmit={handleSavePlan}
      />

      <DailyWorkPlanActionModal
        open={!!actionState.mode}
        ...
        canManage={canManage}
        saving={saving}
        onClose={() => setActionState({ mode: '', plan: null })}
        onSubmit={handleActionSubmit}
      />

      <DailyWorkPlanDetailsModal
        open={!!detailsPlan}
        plan={detailsPlan}
        onClose={() => setDetailsPlan(null)}
        onSendWhatsApp={sendPlanToWhatsApp}
      />
    </>
  );
}
